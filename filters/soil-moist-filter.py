...
configs = dict(config_temp.items('SOIL_MOISTURE'))
print(configs)
high_soil_moist = int(configs['high'])
low_soil_moist = int(configs['low'])
...
rawThings = requests.get(url = host + port + "/things?name=soil-moisture-filter",
                headers = headers)
...
client = mqtt.Client("soil-moist-filter")
